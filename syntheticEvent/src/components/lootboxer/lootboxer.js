...
class Lootboxer extends React.Component {
  constructor(props) {
    ...
    this.handleItemHover = this.handleItemHover.bind(this); // это позволяет вызывать функцию с переданным в качестве параметра контекстом, т.к. по умолчанию методы класса не привязаны к контексту
  }
  ...
  handleItemHover(e) {
    this.setState({ ...this.state, isHovedred: !this.state.isHovedred });
  }
    ...
    return (
      <section className={lootboxerStyles.root}>
        <p className={lootboxerStyles.description}>
          {this.state.currentItem && this.state.isHovedred && (
        ...
        <h2 className={lootboxerStyles.title}>
