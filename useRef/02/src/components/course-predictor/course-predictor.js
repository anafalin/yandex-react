...
const getRandomNum = () =>
  (Math.random() * (MAX_VALUE - MIN_VALUE) + MIN_VALUE).toFixed(2);
// вычисление тренда
...
export default function CoursePredictor() {
  ...
  const isDown =
    getRatio(currentCourse, previousCourse) < 0
      ? coursePredictorStyles.ratioDown
      : coursePredictorStyles.ratioUp;
  ...
  return (
    <section className={coursePredictorStyles.card}>
      ...
      <button
        ...
        className={coursePredictorStyles.button}
      >
        Заглянуть в будущее
      </button>
    </section>
